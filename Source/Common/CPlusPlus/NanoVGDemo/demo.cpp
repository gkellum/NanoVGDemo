...
#include <sstream>
#include <math.h>
// Includes the definition of _NSGetExecutablePath for getting the app's path
...
std::string getPathToApplicationsExecutable()
{
    char path[1024];
...
std::string getPathToResource(const char* filename)
{
    std::string applicationsExecutable(getPathToApplicationsExecutable());
    std::string applicationsExecutablesParentDirectory;
    const size_t last_slash_idx = applicationsExecutable.rfind('/');
    if (std::string::npos != last_slash_idx)
    {
        applicationsExecutablesParentDirectory = applicationsExecutable.substr(0, last_slash_idx);
    }

    std::stringstream ss;
    ss << applicationsExecutablesParentDirectory << "/";
...
int loadDemoData(NVGcontext* vg, DemoData* data)
{
    ...
    for (i = 0; i < 12; i++) {
        char fileName[128];
        snprintf(fileName, 128, "image%d.jpg", i+1);
        std::string file(getPathToResource(fileName));
    ...
    std::string pathToEntypoFont(getPathToResource("entypo.ttf"));
    data->fontIcons = nvgCreateFont(vg, "icons", pathToEntypoFont.c_str());
    if (data->fontIcons == -1) {
        printf("Could not add font icons.\n");
        return -1;
    }
    data->fontNormal = nvgCreateFont(vg, "sans", getPathToResource("Roboto-Regular.ttf").c_str());
    if (data->fontNormal == -1) {
        printf("Could not add font italic.\n");
        return -1;
    }
    data->fontBold = nvgCreateFont(vg, "sans-bold", getPathToResource("Roboto-Bold.ttf").c_str());
    if (data->fontBold == -1) {
